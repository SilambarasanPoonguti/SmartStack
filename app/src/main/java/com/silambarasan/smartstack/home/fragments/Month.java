...
public class Month extends Fragment {
    ...
    @Bind(R.id.err_result_month)
    TextView err_result_questions;
    @Bind(R.id.month_progressBar)
    ProgressBar month_progressBar;

    private String TAG = Month.class.getSimpleName();
    ...
    private void initComponents() {
        try {
            if (isAdded()) {
                ...
                month_progressBar.setVisibility(View.GONE);
    ...
    // questions?page=1&pagesize=10&order=desc&sort=month&site=stackoverflow
    private void getQuestions() {

        month_progressBar.setVisibility(View.VISIBLE);
        err_result_questions.setVisibility(View.GONE);
        String url = Constants.API +
                "questions?page=" + PAGE_COUNT +
                "&pagesize=" + PAGE_SIZE +
                "&order=desc&sort=month&site=stackoverflow";
